...
function fetchCountries(name) {
  return fetch(`https://restcountries.com/v3.1/name/${name}?fields=name,capital,population,flags,languages`).then(resolve => {
    if (!resolve.ok) throw new Error(resolve.status);
    return resolve.json();
  }).catch(error => {
    if (error.message === status.NOT_FOUND) {
      _notiflix.Notify.failure('Oops, there is no country with that name');
    }
  });
}

;
},{"notiflix":"../node_modules/notiflix/dist/notiflix-aio-3.2.2.min.js"}],"../node_modules/lodash.debounce/index.js":[function(require,module,exports) {
var global = arguments[3];
/**
...
function respond(items) {
  if (items.length === 1) {
    const markup = items.map(item => {
      return `<li>
                <p><img src="${item.flags.svg}" alt="Flaga"> ${item.name.official} </p>
  ...
  } else if (items.length > 10) _notiflixNotifyAio.Notify.info('Too many matches found. Please enter a more specific name.');
}
...
searchInput.addEventListener('input', debounce(() => {
  ...
  (0, _fetchCountries.fetchCountries)(names).then(items => respond(items));
}, DEBOUNCE_DELAY));
},{"./css/styles.css":"css/styles.css","./fetchCountries":"fetchCountries.js","lodash.debounce":"../node_modules/lodash.debounce/index.js","notiflix/build/notiflix-notify-aio":"../node_modules/notiflix/build/notiflix-notify-aio.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
